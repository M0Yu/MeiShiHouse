...
public class ShiHeFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		rootView = inflater.inflate(
				R.layout.zmj_personal_shihe_pulltorefresh_scrollview, null);
		initView();
		initData();
		return rootView;
	...
	public void initView() {
		...
		mPullRefreshScrollView = (PullToRefreshScrollView) rootView
				.findViewById(R.id.shihe_pull_refresh_scrollview);
		colOne = (LinearLayout) rootView
				.findViewById(R.id.shiheColOneLinelayout);
		mPullRefreshScrollView
				.setOnRefreshListener(new OnRefreshListener<ScrollView>() {
					...
					public void onRefresh(
						...
						caculateMinMax();
						new GetShiHeCollectionDataTask().execute(
								ZmjConstant.getPersonalShiHeCollectionUrl,
								User_id, min + "", max + "");
	...
	public void initData() {
		...
		new GetShiHeCollectionDataTask().execute(
				ZmjConstant.getPersonalShiHeCollectionUrl, User_id, min + "",
				max + "");
	...
	private void setData() {
		...
		for (int i = 0; i < dataList.size(); i++) {
			...
			view = LayoutInflater.from(getActivity()).inflate(
					R.layout.zmj_shihe_cell, null);

			final int ssss = i;
			view.setLayoutParams(params);
			final FrameLayout closeViewFrameLayout = (FrameLayout) view
					.findViewById(R.id.shihe_closeViewFrameLayout);
			...
			// if (dataList.size() == 4) {
			//
			// ZmjConstant.ShiHeTotalViewShowNumbers = currentPage * pageSize;
			// } else {
			//
			// ZmjConstant.ShiHeTotalViewShowNumbers = dataList.size();
			// }

			// perInfAc.setOnDeleteShiHeCollectionListener(new
			// OnDeleteShiHeCollectionListener() {
			//
			// @Override
			// public void OnIsReadyToDeleteShiHeCollection() {
			// // TODO Auto-generated method stub
			...
			// ZmjConstant.whichFragmentIsDoingDeleteOperation = 1;
			//
			...
			// }
			//
			// ZmjConstant.ShiHeAlreadyShowFrameLayoutNumbersofView =
			...
			//
			// @Override
			// public void OnCompletedDeleteShiHeCollection() {
			// // TODO Auto-generated method stub
			// // for (FrameLayout a : framelayoutArrayList) {
			...
			// // }
			//
			// ZmjConstant.ShiHeAlreadyShowFrameLayoutNumbersofView = 0;
			// ZmjConstant.ShiHeTotalViewShowNumbers = 4;
			...
			// max = 4;
			// new GetShiHeCollectionDataTask().execute(
			// ZmjConstant.getPersonalShiHeCollectionUrl, User_id,
			// min + "", max + "");
			//
			// }
			// });
			...
			view.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if (flag.get(closeViewFrameLayout) == 0) {
						...
						new DeleteCollectonTask(temp - 1).execute(
								ZmjConstant.deletePersonalShiHeCollectionUrl,
								User_id,
								dataSumList.get(temp - 1).get("store_list_id"));
			...
			RoundRectImageView picture = (RoundRectImageView) view
					.findViewById(R.id.shiHeRectImageViewPic);
			ImageView shiheCollectionDivider = (ImageView) view
					.findViewById(R.id.shiheCollectionDivider);
			shiheCollectionDivider
		...
		perInfAc.setOnDeleteShiHeCollectionListener(new OnDeleteShiHeCollectionListener() {
			...
			public void OnIsReadyToDeleteShiHeCollection() {
				...
				ZmjConstant.ShiHeAlreadyShowFrameLayoutNumbersofView = framelayoutArrayList
						.size();
		...
		ZmjConstant.ShiHeTotalViewShowNumbers = dataSumList.size();
	}
	...
	private class DeleteCollectonTask extends AsyncTask<String, String, String> {
		...
		protected void onPostExecute(String result) {
			...
			if (result.equals("true")) {
				...
				ZmjConstant.ShiHeAlreadyShowFrameLayoutNumbersofView -= 1;
				ZmjConstant.ShiHeTotalViewShowNumbers -= 1;
	...
	// 异步线程 这里获取数据
	private class GetShiHeCollectionDataTask extends
		...
		protected void onPostExecute(String result) {
			...
			dataList = zmjAnalysisTools.getPersonalShiHeCollectionDataList();
			// 异步任务完成后 ，适配数据
			setData();
